...
case "$1" in
    start)
        ...
        nohup python3 $PATH_SCRIPT --interface noop --display linux_framebuffer --display-theme blue_new.cfg/green_new.cfg --display-offset 0/160 --follow F1ZPX/F4HWN > $PATH_PID/RRFDisplay_0.log 2>&1 & echo $! > $PATH_PID/RRFDisplay_0.pid
       ;;
    stop) 

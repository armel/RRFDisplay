...
case "$1" in
    start)
        ...
        nohup python3 $PATH_SCRIPT --interface i2c --i2c-port 1 --display ssd1327 --display-height 128 --display-theme gray.cfg --follow F4HWN > $PATH_PID/RRFDisplay_1.log 2>&1 & echo $! > $PATH_PID/RRFDisplay_1.pid
        ;;
    stop) 
